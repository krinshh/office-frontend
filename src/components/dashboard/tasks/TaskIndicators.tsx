...
export const ProgressIndicator = ({ assignments }: { assignments: any[] }) => {
  const t = useTranslations();
  ...
  return (
    <div className="flex items-center gap-2 mb-2">
      <div className="flex-1 bg-muted rounded-full h-1.5 overflow-hidden">
        <div
          className="h-full bg-gradient-to-r from-secondary to-secondary/80 rounded-full transition-all duration-500 ease-out"
          style={{ width: `${percentage}%` }}
        ></div>
      </div>
      <span className="text-xs text-muted-foreground font-medium">
        {t('tasks.progressText', { completed, total, percentage })}
      </span>
    </div>
  );
};
